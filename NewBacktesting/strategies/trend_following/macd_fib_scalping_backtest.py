...
class MACD_Fib_Scalping(Strategy):
    # Strategy parameters
    fast_period = 12
    slow_period = 26
    signal_period = 9
    ...
    def init(self):
        ...
        self.macd_line, self.signal_line, self.histogram = self.I(macd, 
                                                                 self.data.Close,
                                                                 self.fast_period,
                                                                 self.slow_period,
                                                                 self.signal_period)
...
if __name__ == "__main__":
    ...
    for timeframe, tokens in available_data.items():
        for token in tokens:
            logger.info(f"Running macd_fib_scalping backtest for {token} on {timeframe} timeframe...")
            run_all_timeframes(
                strategy_class=MACD_Fib_Scalping,
                strategy_name='macd_fib_scalping',
                symbols=[token],
