...
class EMA_Fib_Scalping(Strategy):
    # Strategy parameters
    fast_ema = 8
    slow_ema = 21
    ...
    def init(self):
        # Calculate EMAs
        self.fast_ema = self.I(lambda x: pd.Series(x).ewm(span=self.fast_ema, adjust=False).mean(), self.data.Close)
        self.slow_ema = self.I(lambda x: pd.Series(x).ewm(span=self.slow_ema, adjust=False).mean(), self.data.Close)
    ...
    def next(self):
        ...
        if self.position.is_long:
            ...
            elif (self.fast_ema[-1] < self.slow_ema[-1] and 
        ...
        # Entry conditions
            ...
            if (self.fast_ema[-1] > self.slow_ema[-1] and
...
if __name__ == "__main__":
    ...
    for timeframe, tokens in available_data.items():
        for token in tokens:
            logger.info(f"Running ema_fib_scalping backtest for {token} on {timeframe} timeframe...")
            run_all_timeframes(
                strategy_class=EMA_Fib_Scalping,
                strategy_name='ema_fib_scalping',
                symbols=[token],
