...
class Bollinger_Fib_Scalping(Strategy):
    # Strategy parameters
    bb_period = 20
    bb_std = 2.0
    ...
    def init(self):
        ...
        self.sma, self.upper, self.lower, self.std_dev = self.I(bollinger_bands, 
                                                               self.data.Close,
                                                               self.bb_period,
                                                               self.bb_std)
...
if __name__ == "__main__":
    ...
    for timeframe, tokens in available_data.items():
        for token in tokens:
            logger.info(f"Running bollinger_fib_scalping backtest for {token} on {timeframe} timeframe...")
            run_all_timeframes(
                strategy_class=Bollinger_Fib_Scalping,
                strategy_name='bollinger_fib_scalping',
                symbols=[token],
