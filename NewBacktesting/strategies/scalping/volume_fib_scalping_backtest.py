...
class Volume_Fib_Scalping(Strategy):
    # Strategy parameters
    volume_ma_period = 20
    ...
    def init(self):
        ...
        self.volume_ma = self.I(lambda x: pd.Series(x).rolling(window=self.volume_ma_period).mean(), 
                               self.data.Volume)
        
        # Calculate Volume Ratio (current volume / average volume)
        self.volume_ratio = self.I(lambda x, y: x / y, 
                                  self.data.Volume, 
                                  self.volume_ma)
    ...
    def next(self):
        ...
        if self.position.is_long:
            ...
            elif (volume_ratio < 1.0 and  # Volume decreasing
        ...
        # Entry conditions
            ...
            if (volume_ratio > self.volume_threshold and  # High volume
            ...
            elif (volume_ratio > self.volume_threshold and  # High volume
...
if __name__ == "__main__":
    ...
    for timeframe, tokens in available_data.items():
        for token in tokens:
            logger.info(f"Running volume_fib_scalping backtest for {token} on {timeframe} timeframe...")
            run_all_timeframes(
                strategy_class=Volume_Fib_Scalping,
                strategy_name='volume_fib_scalping',
                symbols=[token],
