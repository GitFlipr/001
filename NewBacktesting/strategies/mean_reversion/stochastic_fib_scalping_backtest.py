...
class Stochastic_Fib_Scalping(Strategy):
    # Strategy parameters
    k_period = 14
    d_period = 3
    stoch_overbought = 80
    ...
    def init(self):
        ...
        self.stoch_k, self.stoch_d = self.I(stochastic, 
                                          self.data.High, 
                                          self.data.Low, 
                                          self.data.Close,
                                          self.k_period,
    ...
    def next(self):
        ...
        if self.position.is_long:
            ...
            elif (self.stoch_k[-1] > self.stoch_overbought and 
                  self.stoch_d[-1] > self.stoch_overbought and
            ...
            # Check stop loss
                ...
                self.position_size = 0
            # Check exit conditions
            elif (self.stoch_k[-1] < self.stoch_oversold and 
                  self.stoch_d[-1] < self.stoch_oversold and
                  price > fib_levels[0.5]):
        ...
        # Entry conditions
            ...
            if (self.stoch_k[-1] < self.stoch_oversold and
                self.stoch_d[-1] < self.stoch_oversold and
                self.stoch_k[-1] > self.stoch_k[-2] and  # K line starting to rise
                self.stoch_d[-1] > self.stoch_d[-2] and  # D line starting to rise
            ...
            # Short entry conditions
                  ...
                  self.stoch_d[-1] > self.stoch_overbought and
                  self.stoch_k[-1] < self.stoch_k[-2] and  # K line starting to fall
                  self.stoch_d[-1] < self.stoch_d[-2] and  # D line starting to fall
                  price < fib_levels[0.5] and
                  volume > self.min_volume and 
                  volatility > self.min_volatility):
...
if __name__ == "__main__":
    ...
    for timeframe, tokens in available_data.items():
        for token in tokens:
            logger.info(f"Running stochastic_fib_scalping backtest for {token} on {timeframe} timeframe...")
            run_all_timeframes(
                strategy_class=Stochastic_Fib_Scalping,
                strategy_name='stochastic_fib_scalping',
                symbols=[token],
