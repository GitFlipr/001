...
class RSI_Fib_Scalping(Strategy):
    # Strategy parameters
    rsi_period = 14
    rsi_overbought = 70
    rsi_oversold = 30
    ...
    def next(self):
        ...
        # Entry conditions
            ...
            if (self.rsi[-1] < self.rsi_oversold and
                self.rsi[-1] > self.rsi[-2] and  # RSI starting to rise
            ...
            # Short entry conditions
                  ...
                  self.rsi[-1] < self.rsi[-2] and  # RSI starting to fall
                  price < fib_levels[0.5] and
                  volume > self.min_volume and 
                  volatility > self.min_volatility):
...
if __name__ == "__main__":
    ...
    for timeframe, tokens in available_data.items():
        for token in tokens:
            logger.info(f"Running rsi_fib_scalping backtest for {token} on {timeframe} timeframe...")
            run_all_timeframes(
                strategy_class=RSI_Fib_Scalping,
                strategy_name='rsi_fib_scalping',
                symbols=[token],
