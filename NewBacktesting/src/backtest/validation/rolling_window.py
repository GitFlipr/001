...
Trains on fixed rolling training window and tests on next period, rolling forward.
"""
import sys
from pathlib import Path

# Allow running this file directly as a script (python rolling_window.py)
...
class RollingWindowValidator:
    """
    Rolling window validation using fixed training and test windows.
    """
    def __init__(
        self,
        data: pd.DataFrame,
    ...
    def run(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not splits:
            return {"error": "Insufficient data for rolling window validation"}
            
        # Run each rolling window iteration
        for i, split_idx in enumerate(splits):
            # Split data
            train_start = split_idx - self.train_window
            train_data = self.data.iloc[train_start:split_idx]
        ...
        if not results:
            return {"error": "No valid rolling window iterations completed"}
